!--------------------3D block tridi solvers---------------------------------

!--------i direction solver-------------------------------------------------

SUBROUTINE BLKTRI31(numVar,      &
                    iSolveStart, &
                    iSolveEnd,   &
                    aStart,      &
                    A,           &
...
!     C
      ...
      INTEGER, DIMENSION(:), INTENT(IN) :: iSolveStart, & ! dimension 3
                                           iSolveEnd,   &
                                           aStart,      &
                                           bStart,      &
                                           cStart,      &
...
!     .....NORMALIZE PIVOT ROW
        ...
        T=1.0_rDef/B(I,J,K,M,M)

        DO N=M+1,numVar
         B(I,J,K,M,N) = T*B(I,J,K,M,N)
        END DO

        DO N=1,numVar
         C(I,J,K,M,N) = T*C(I,J,K,M,N)
        END DO

        R(I,J,K,M) = T*R(I,J,K,M)
...
!     .....LOWER ELIMINATION IN B
        ...
        DO N=M+1,numVar
         ...
         DO NN=M+1,numVar
          B(I,J,K,N,NN) = B(I,J,K,N,NN)-B(I,J,K,N,M)*B(I,J,K,M,NN)
         END DO

         DO NN=1,numVar
          C(I,J,K,N,NN) = C(I,J,K,N,NN)-B(I,J,K,N,M)*C(I,J,K,M,NN)
         END DO
         R(I,J,K,N) = R(I,J,K,N)-B(I,J,K,N,M)*R(I,J,K,M)
        END DO ! n loop

       END DO ! m loop

!     .....UPPER (GAUSS JORDAN) ELIMINATION IN B
       T= 1.0_rDef/B(I,J,K,numVar,numVar)

       DO NN = 1,numVar
        C(I,J,K,numVar,NN) = T*C(I,J,K,numVar,NN)
       END DO

       R(I,J,K,numVar) = T*R(I,J,K,numVar)

       DO N=numVar,2,-1
        DO M=N-1,1,-1
         DO NN=1,numVar
          C(I,J,K,M,NN) = C(I,J,K,M,NN)-B(I,J,K,M,N)*C(I,J,K,N,NN)
         END DO

         R(I,J,K,M) = R(I,J,K,M)-B(I,J,K,M,N)*R(I,J,K,N)
        END DO ! m loop
       END DO  ! n loop
...
!     .....ELIMINATE A(I+1)
       DO N=1,numVar
        DO M=1,numVar
         DO NN=1,numVar
          B(I+1,J,K,M,NN) = B(I+1,J,K,M,NN)-A(I+1,J,K,M,N)*C(I,J,K,N,NN)
         END DO ! nn loop
         R(I+1,J,K,M) = R(I+1,J,K,M)-A(I+1,J,K,M,N)*R(I,J,K,N)
        END DO ! m loop
       END DO ! n loop
      END DO  ! i loop
...
!     .....BACK SUBSTITUTION
      ...
      DO I=iSolveEnd(1)-1,iSolveStart(1),-1
       DO M=numVar,1,-1
        S=0.0_rDef
        DO NN=1,numVar
         S = C(I,J,K,M,NN)*X(I+1,J,K,NN)+S
        END DO ! nn loop
        X(I,J,K,M) = R(I,J,K,M)-S
       END DO ! m loop
      END DO ! i loop
     END DO  ! j loop
    END DO  ! k loop
    ...
    RETURN
END SUBROUTINE BLKTRI31

!--------j direction solver-------------------------------------------------

SUBROUTINE BLKTRI32(numVar,      &
                    iSolveStart, &
                    iSolveEnd,   &
                    aStart,      &
                    A,           &
...
!     .....ELIMINATE A(I+1)
       DO N=1,numVar
        DO M=1,numVar
         DO NN=1,numVar
          B(I,J+1,K,M,NN) = B(I,J+1,K,M,NN)-A(I,J+1,K,M,N)*C(I,J,K,N,NN)
         END DO ! nn loop
         R(I,J+1,K,M) = R(I,J+1,K,M)-A(I,J+1,K,M,N)*R(I,J,K,N)
        END DO ! m loop
       END DO ! n loop
      END DO  ! i loop
     END DO  ! J loop
...
!     .....BACK SUBSTITUTION
    ...
    RETURN
END SUBROUTINE BLKTRI32

!--------k direction solver-------------------------------------------------

SUBROUTINE BLKTRI33(numVar,      &
                    iSolveStart, &
                    iSolveEnd,   &
                    aStart,      &
                    A,           &
...
!     .....ELIMINATE A(I+1)
       DO N=1,numVar
        DO M=1,numVar
         DO NN=1,numVar
          B(I,J,K+1,M,NN) = B(I,J,K+1,M,NN)-A(I,J,K+1,M,N)*C(I,J,K,N,NN)
         END DO ! nn loop
         R(I,J,K+1,M) = R(I,J,K+1,M)-A(I,J,K+1,M,N)*R(I,J,K,N)
        END DO ! m loop
       END DO ! n loop
      END DO  ! i loop
     END DO  ! J loop
