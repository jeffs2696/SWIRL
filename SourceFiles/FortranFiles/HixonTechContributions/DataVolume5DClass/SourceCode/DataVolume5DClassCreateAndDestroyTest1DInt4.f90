...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int4BufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = int4Data1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int4DataVolume5DObject,              &
                              pointerToVolumeData     = int4Ptr1D,                           &
                              pointerDataTotalBounds  = int4PointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = int4PointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int4Ptr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int4Ptr1D,1),UBOUND(int4Ptr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4Ptr1D,        &
                      expectedDataValue = int4Data1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int4Data1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int4BufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = int4BufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = int4BufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
