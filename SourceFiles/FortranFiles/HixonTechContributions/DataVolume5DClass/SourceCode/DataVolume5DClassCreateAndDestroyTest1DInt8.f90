...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int8BufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = int8Data1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int8DataVolume5DObject,              &
                              pointerToVolumeData     = int8Ptr1D,                           &
                              pointerDataTotalBounds  = int8PointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = int8PointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int8Ptr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int8Ptr1D,1),UBOUND(int8Ptr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8Ptr1D,        &
                      expectedDataValue = int8Data1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int8Data1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int8BufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = int8BufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = int8BufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
