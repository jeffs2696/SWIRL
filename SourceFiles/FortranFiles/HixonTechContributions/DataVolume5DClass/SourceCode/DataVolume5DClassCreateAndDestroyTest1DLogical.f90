...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = logicalDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = logicalData1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = logicalDataVolume5DObject,              &
                              pointerToVolumeData     = logicalPtr1D,                           &
                              pointerDataTotalBounds  = logicalPointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = logicalPointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: logicalPtr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(logicalPtr1D,1),UBOUND(logicalPtr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalPtr1D,        &
                      expectedDataValue = logicalData1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect logicalData1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = logicalDataVolume5DObject,                 &
                           volumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = logicalBufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = logicalBufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
