...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real4BufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = real4Data1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real4DataVolume5DObject,              &
                              pointerToVolumeData     = real4Ptr1D,                           &
                              pointerDataTotalBounds  = real4PointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = real4PointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real4Ptr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real4Ptr1D,1),UBOUND(real4Ptr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4Ptr1D,        &
                      expectedDataValue = real4Data1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real4Data1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real4BufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = real4BufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = real4BufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
