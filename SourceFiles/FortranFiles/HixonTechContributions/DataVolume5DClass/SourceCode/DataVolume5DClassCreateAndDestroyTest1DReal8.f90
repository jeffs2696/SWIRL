...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real8BufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = real8Data1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real8DataVolume5DObject,              &
                              pointerToVolumeData     = real8Ptr1D,                           &
                              pointerDataTotalBounds  = real8PointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = real8PointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real8Ptr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real8Ptr1D,1),UBOUND(real8Ptr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8Ptr1D,        &
                      expectedDataValue = real8Data1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real8Data1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real8BufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = real8BufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = real8BufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
