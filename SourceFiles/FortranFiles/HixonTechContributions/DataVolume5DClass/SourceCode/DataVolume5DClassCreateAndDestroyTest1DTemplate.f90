...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = varTypeDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds1D,  &
                    bufferVolumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds1D, &
                    bufferVolumeData             = varTypeData1D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = varTypeDataVolume5DObject,              &
                              pointerToVolumeData     = varTypePtr1D,                           &
                              pointerDataTotalBounds  = varTypePointerVolumeDataTotalBounds1D,  &
                              pointerDataUpdateBounds = varTypePointerVolumeDataUpdateBounds1D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: varTypePtr1D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(varTypePtr1D,1),UBOUND(varTypePtr1D,1)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypePtr1D,        &
                      expectedDataValue = varTypeData1D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect varTypeData1D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = varTypeDataVolume5DObject,                 &
                           volumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds1DTest,  &
                           volumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds1DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataTotalBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataTotalBounds1DTest, &
                      expectedDataValue = varTypeBufferVolumeDataTotalBounds1D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataUpdateBounds1DTest(1,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataUpdateBounds1DTest, &
                      expectedDataValue = varTypeBufferVolumeDataUpdateBounds1D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
