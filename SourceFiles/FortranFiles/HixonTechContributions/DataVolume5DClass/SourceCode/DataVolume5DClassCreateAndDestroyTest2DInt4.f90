...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int4BufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = int4Data2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int4DataVolume5DObject,              &
                              pointerToVolumeData     = int4Ptr2D,                           &
                              pointerDataTotalBounds  = int4PointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = int4PointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int4Ptr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int4Ptr2D,1),UBOUND(int4Ptr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int4Ptr2D,2),UBOUND(int4Ptr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4Ptr2D,        &
                      expectedDataValue = int4Data2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   WRITE(charStringObject%charString,'(a11,4(1x,i5))') ' Location: ',errorLocation(1:2)
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)

   charStringObject%charString = 'ERROR:  incorrect int4Data2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int4BufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = int4BufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = int4BufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
