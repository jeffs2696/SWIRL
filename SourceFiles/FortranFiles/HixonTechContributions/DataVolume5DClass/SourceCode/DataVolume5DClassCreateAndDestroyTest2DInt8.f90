...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int8BufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = int8Data2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int8DataVolume5DObject,              &
                              pointerToVolumeData     = int8Ptr2D,                           &
                              pointerDataTotalBounds  = int8PointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = int8PointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int8Ptr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int8Ptr2D,1),UBOUND(int8Ptr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int8Ptr2D,2),UBOUND(int8Ptr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8Ptr2D,        &
                      expectedDataValue = int8Data2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int8Data2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int8BufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = int8BufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = int8BufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
