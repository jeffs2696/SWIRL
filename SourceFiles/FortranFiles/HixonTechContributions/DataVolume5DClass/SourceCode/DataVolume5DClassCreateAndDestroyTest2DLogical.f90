...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = logicalDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = logicalData2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = logicalDataVolume5DObject,              &
                              pointerToVolumeData     = logicalPtr2D,                           &
                              pointerDataTotalBounds  = logicalPointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = logicalPointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: logicalPtr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(logicalPtr2D,1),UBOUND(logicalPtr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(logicalPtr2D,2),UBOUND(logicalPtr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalPtr2D,        &
                      expectedDataValue = logicalData2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect logicalData2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = logicalDataVolume5DObject,                 &
                           volumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',logicalBufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = logicalBufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',logicalBufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = logicalBufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
