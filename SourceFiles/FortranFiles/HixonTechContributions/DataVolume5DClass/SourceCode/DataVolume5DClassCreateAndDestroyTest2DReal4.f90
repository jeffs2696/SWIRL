...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real4BufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = real4Data2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real4DataVolume5DObject,              &
                              pointerToVolumeData     = real4Ptr2D,                           &
                              pointerDataTotalBounds  = real4PointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = real4PointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real4Ptr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real4Ptr2D,1),UBOUND(real4Ptr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real4Ptr2D,2),UBOUND(real4Ptr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4Ptr2D,        &
                      expectedDataValue = real4Data2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real4Data2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real4BufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = real4BufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = real4BufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
