...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real8BufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = real8Data2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real8DataVolume5DObject,              &
                              pointerToVolumeData     = real8Ptr2D,                           &
                              pointerDataTotalBounds  = real8PointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = real8PointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real8Ptr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real8Ptr2D,1),UBOUND(real8Ptr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real8Ptr2D,2),UBOUND(real8Ptr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8Ptr2D,        &
                      expectedDataValue = real8Data2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real8Data2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real8BufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = real8BufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = real8BufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
