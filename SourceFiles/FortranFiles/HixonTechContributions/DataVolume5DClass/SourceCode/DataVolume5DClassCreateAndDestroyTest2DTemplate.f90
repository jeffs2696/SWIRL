...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = varTypeDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds2D,  &
                    bufferVolumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds2D, &
                    bufferVolumeData             = varTypeData2D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = varTypeDataVolume5DObject,              &
                              pointerToVolumeData     = varTypePtr2D,                           &
                              pointerDataTotalBounds  = varTypePointerVolumeDataTotalBounds2D,  &
                              pointerDataUpdateBounds = varTypePointerVolumeDataUpdateBounds2D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: varTypePtr2D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(varTypePtr2D,1),UBOUND(varTypePtr2D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(varTypePtr2D,2),UBOUND(varTypePtr2D,2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypePtr2D,        &
                      expectedDataValue = varTypeData2D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect varTypeData2D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = varTypeDataVolume5DObject,                 &
                           volumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds2DTest,  &
                           volumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds2DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataTotalBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',varTypeBufferVolumeDataTotalBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataTotalBounds2DTest, &
                      expectedDataValue = varTypeBufferVolumeDataTotalBounds2D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataUpdateBounds2DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',varTypeBufferVolumeDataUpdateBounds2DTest(2,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataUpdateBounds2DTest, &
                      expectedDataValue = varTypeBufferVolumeDataUpdateBounds2D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
