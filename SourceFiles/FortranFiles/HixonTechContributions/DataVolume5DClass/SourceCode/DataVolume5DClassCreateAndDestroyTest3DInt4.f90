...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int4BufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = int4Data3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int4DataVolume5DObject,              &
                              pointerToVolumeData     = int4Ptr3D,                           &
                              pointerDataTotalBounds  = int4PointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = int4PointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int4Ptr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int4Ptr3D,1),UBOUND(int4Ptr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int4Ptr3D,2),UBOUND(int4Ptr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(int4Ptr3D,3),UBOUND(int4Ptr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4Ptr3D,        &
                      expectedDataValue = int4Data3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   WRITE(charStringObject%charString,'(a11,4(1x,i5))') ' Location: ',errorLocation(1:3)
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)

   charStringObject%charString = 'ERROR:  incorrect int4Data3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int4BufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int4BufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = int4BufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int4BufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = int4BufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
