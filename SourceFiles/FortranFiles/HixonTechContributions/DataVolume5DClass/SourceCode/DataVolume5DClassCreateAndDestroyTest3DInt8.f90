...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int8BufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = int8Data3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int8DataVolume5DObject,              &
                              pointerToVolumeData     = int8Ptr3D,                           &
                              pointerDataTotalBounds  = int8PointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = int8PointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int8Ptr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int8Ptr3D,1),UBOUND(int8Ptr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int8Ptr3D,2),UBOUND(int8Ptr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(int8Ptr3D,3),UBOUND(int8Ptr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8Ptr3D,        &
                      expectedDataValue = int8Data3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int8Data3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int8BufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = int8BufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = int8BufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
