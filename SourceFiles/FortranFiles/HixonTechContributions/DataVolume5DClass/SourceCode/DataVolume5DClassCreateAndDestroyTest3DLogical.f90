...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = logicalDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = logicalData3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = logicalDataVolume5DObject,              &
                              pointerToVolumeData     = logicalPtr3D,                           &
                              pointerDataTotalBounds  = logicalPointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = logicalPointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: logicalPtr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(logicalPtr3D,1),UBOUND(logicalPtr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(logicalPtr3D,2),UBOUND(logicalPtr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(logicalPtr3D,3),UBOUND(logicalPtr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalPtr3D,        &
                      expectedDataValue = logicalData3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect logicalData3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = logicalDataVolume5DObject,                 &
                           volumeDataTotalBounds  = logicalBufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = logicalBufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',logicalBufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',logicalBufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = logicalBufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',logicalBufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',logicalBufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',logicalBufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = logicalBufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = logicalBufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
