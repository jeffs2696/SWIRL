...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real4BufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = real4Data3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real4DataVolume5DObject,              &
                              pointerToVolumeData     = real4Ptr3D,                           &
                              pointerDataTotalBounds  = real4PointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = real4PointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real4Ptr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real4Ptr3D,1),UBOUND(real4Ptr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real4Ptr3D,2),UBOUND(real4Ptr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(real4Ptr3D,3),UBOUND(real4Ptr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4Ptr3D,        &
                      expectedDataValue = real4Data3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real4Data3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real4BufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real4BufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = real4BufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real4BufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = real4BufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
