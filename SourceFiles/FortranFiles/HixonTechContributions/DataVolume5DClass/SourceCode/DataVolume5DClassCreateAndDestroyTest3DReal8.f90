...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real8BufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = real8Data3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real8DataVolume5DObject,              &
                              pointerToVolumeData     = real8Ptr3D,                           &
                              pointerDataTotalBounds  = real8PointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = real8PointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real8Ptr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real8Ptr3D,1),UBOUND(real8Ptr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real8Ptr3D,2),UBOUND(real8Ptr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(real8Ptr3D,3),UBOUND(real8Ptr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8Ptr3D,        &
                      expectedDataValue = real8Data3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real8Data3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real8BufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real8BufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = real8BufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real8BufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = real8BufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
