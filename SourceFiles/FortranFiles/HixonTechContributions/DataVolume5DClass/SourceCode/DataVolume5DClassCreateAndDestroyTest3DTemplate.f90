...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = varTypeDataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds3D,  &
                    bufferVolumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds3D, &
                    bufferVolumeData             = varTypeData3D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = varTypeDataVolume5DObject,              &
                              pointerToVolumeData     = varTypePtr3D,                           &
                              pointerDataTotalBounds  = varTypePointerVolumeDataTotalBounds3D,  &
                              pointerDataUpdateBounds = varTypePointerVolumeDataUpdateBounds3D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: varTypePtr3D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(varTypePtr3D,1),UBOUND(varTypePtr3D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(varTypePtr3D,2),UBOUND(varTypePtr3D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(varTypePtr3D,3),UBOUND(varTypePtr3D,3)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypePtr3D,        &
                      expectedDataValue = varTypeData3D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect varTypeData3D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = varTypeDataVolume5DObject,                 &
                           volumeDataTotalBounds  = varTypeBufferVolumeDataTotalBounds3DTest,  &
                           volumeDataUpdateBounds = varTypeBufferVolumeDataUpdateBounds3DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataTotalBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',varTypeBufferVolumeDataTotalBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',varTypeBufferVolumeDataTotalBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataTotalBounds3DTest, &
                      expectedDataValue = varTypeBufferVolumeDataTotalBounds3D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',varTypeBufferVolumeDataUpdateBounds3DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',varTypeBufferVolumeDataUpdateBounds3DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',varTypeBufferVolumeDataUpdateBounds3DTest(3,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = varTypeBufferVolumeDataUpdateBounds3DTest, &
                      expectedDataValue = varTypeBufferVolumeDataUpdateBounds3D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
