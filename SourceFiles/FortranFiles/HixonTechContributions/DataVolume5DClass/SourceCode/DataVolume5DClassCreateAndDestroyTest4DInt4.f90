...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int4BufferVolumeDataTotalBounds4D,  &
                    bufferVolumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds4D, &
                    bufferVolumeData             = int4Data4D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int4DataVolume5DObject,              &
                              pointerToVolumeData     = int4Ptr4D,                           &
                              pointerDataTotalBounds  = int4PointerVolumeDataTotalBounds4D,  &
                              pointerDataUpdateBounds = int4PointerVolumeDataUpdateBounds4D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int4Ptr4D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int4Ptr4D,1),UBOUND(int4Ptr4D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int4Ptr4D,2),UBOUND(int4Ptr4D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(int4Ptr4D,3),UBOUND(int4Ptr4D,3)
   WRITE(0,*) '  dimension 4: ',LBOUND(int4Ptr4D,4),UBOUND(int4Ptr4D,4)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4Ptr4D,        &
                      expectedDataValue = int4Data4D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int4Data4D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int4BufferVolumeDataTotalBounds4DTest,  &
                           volumeDataUpdateBounds = int4BufferVolumeDataUpdateBounds4DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataTotalBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataTotalBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int4BufferVolumeDataTotalBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int4BufferVolumeDataTotalBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataTotalBounds4DTest, &
                      expectedDataValue = int4BufferVolumeDataTotalBounds4D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int4BufferVolumeDataUpdateBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int4BufferVolumeDataUpdateBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int4BufferVolumeDataUpdateBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int4BufferVolumeDataUpdateBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int4BufferVolumeDataUpdateBounds4DTest, &
                      expectedDataValue = int4BufferVolumeDataUpdateBounds4D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
