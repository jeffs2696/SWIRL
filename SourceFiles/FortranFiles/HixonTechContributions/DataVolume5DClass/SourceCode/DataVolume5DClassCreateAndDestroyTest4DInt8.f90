...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int8BufferVolumeDataTotalBounds4D,  &
                    bufferVolumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds4D, &
                    bufferVolumeData             = int8Data4D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int8DataVolume5DObject,              &
                              pointerToVolumeData     = int8Ptr4D,                           &
                              pointerDataTotalBounds  = int8PointerVolumeDataTotalBounds4D,  &
                              pointerDataUpdateBounds = int8PointerVolumeDataUpdateBounds4D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int8Ptr4D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int8Ptr4D,1),UBOUND(int8Ptr4D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int8Ptr4D,2),UBOUND(int8Ptr4D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(int8Ptr4D,3),UBOUND(int8Ptr4D,3)
   WRITE(0,*) '  dimension 4: ',LBOUND(int8Ptr4D,4),UBOUND(int8Ptr4D,4)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8Ptr4D,        &
                      expectedDataValue = int8Data4D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int8Data4D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int8BufferVolumeDataTotalBounds4DTest,  &
                           volumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds4DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataTotalBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataTotalBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataTotalBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int8BufferVolumeDataTotalBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataTotalBounds4DTest, &
                      expectedDataValue = int8BufferVolumeDataTotalBounds4D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataUpdateBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataUpdateBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataUpdateBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int8BufferVolumeDataUpdateBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataUpdateBounds4DTest, &
                      expectedDataValue = int8BufferVolumeDataUpdateBounds4D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
