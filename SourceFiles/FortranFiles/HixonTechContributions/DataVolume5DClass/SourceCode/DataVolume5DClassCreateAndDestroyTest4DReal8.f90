...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real8BufferVolumeDataTotalBounds4D,  &
                    bufferVolumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds4D, &
                    bufferVolumeData             = real8Data4D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real8DataVolume5DObject,              &
                              pointerToVolumeData     = real8Ptr4D,                           &
                              pointerDataTotalBounds  = real8PointerVolumeDataTotalBounds4D,  &
                              pointerDataUpdateBounds = real8PointerVolumeDataUpdateBounds4D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real8Ptr4D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real8Ptr4D,1),UBOUND(real8Ptr4D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real8Ptr4D,2),UBOUND(real8Ptr4D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(real8Ptr4D,3),UBOUND(real8Ptr4D,3)
   WRITE(0,*) '  dimension 4: ',LBOUND(real8Ptr4D,4),UBOUND(real8Ptr4D,4)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8Ptr4D,        &
                      expectedDataValue = real8Data4D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real8Data4D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real8BufferVolumeDataTotalBounds4DTest,  &
                           volumeDataUpdateBounds = real8BufferVolumeDataUpdateBounds4DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataTotalBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataTotalBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real8BufferVolumeDataTotalBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',real8BufferVolumeDataTotalBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataTotalBounds4DTest, &
                      expectedDataValue = real8BufferVolumeDataTotalBounds4D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real8BufferVolumeDataUpdateBounds4DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real8BufferVolumeDataUpdateBounds4DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real8BufferVolumeDataUpdateBounds4DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',real8BufferVolumeDataUpdateBounds4DTest(4,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real8BufferVolumeDataUpdateBounds4DTest, &
                      expectedDataValue = real8BufferVolumeDataUpdateBounds4D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
