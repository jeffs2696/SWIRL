!--------------create a dataVolume5DClass object------------------------------
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '------------------------------------'
   WRITE(0,*) 'Testing int8DataVolume5DObject...'
   WRITE(0,*)
  END IF
...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = int8DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = int8BufferVolumeDataTotalBounds5D,  &
                    bufferVolumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds5D, &
                    bufferVolumeData             = int8Data5D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = int8DataVolume5DObject,              &
                              pointerToVolumeData     = int8Ptr5D,                           &
                              pointerDataTotalBounds  = int8PointerVolumeDataTotalBounds5D,  &
                              pointerDataUpdateBounds = int8PointerVolumeDataUpdateBounds5D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: int8Ptr5D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(int8Ptr5D,1),UBOUND(int8Ptr5D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(int8Ptr5D,2),UBOUND(int8Ptr5D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(int8Ptr5D,3),UBOUND(int8Ptr5D,3)
   WRITE(0,*) '  dimension 4: ',LBOUND(int8Ptr5D,4),UBOUND(int8Ptr5D,4)
   WRITE(0,*) '  dimension 5: ',LBOUND(int8Ptr5D,5),UBOUND(int8Ptr5D,5)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8Ptr5D,        &
                      expectedDataValue = int8Data5D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect int8Data5D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
  ...
  ELSE
   IF (masterNode .AND. verbose) THEN
    WRITE(0,*) '...correct value for interior int8Data...'
    WRITE(0,*) 'Test PASSED.'
    WRITE(0,*) '--------------------------------'
    WRITE(0,*)
   END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = int8DataVolume5DObject,                 &
                           volumeDataTotalBounds  = int8BufferVolumeDataTotalBounds5DTest,  &
                           volumeDataUpdateBounds = int8BufferVolumeDataUpdateBounds5DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataTotalBounds5DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataTotalBounds5DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataTotalBounds5DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int8BufferVolumeDataTotalBounds5DTest(4,1:2)
   WRITE(0,*) '  dimension 5: ',int8BufferVolumeDataTotalBounds5DTest(5,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataTotalBounds5DTest, &
                      expectedDataValue = int8BufferVolumeDataTotalBounds5D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',int8BufferVolumeDataUpdateBounds5DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',int8BufferVolumeDataUpdateBounds5DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',int8BufferVolumeDataUpdateBounds5DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',int8BufferVolumeDataUpdateBounds5DTest(4,1:2)
   WRITE(0,*) '  dimension 5: ',int8BufferVolumeDataUpdateBounds5DTest(5,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = int8BufferVolumeDataUpdateBounds5DTest, &
                      expectedDataValue = int8BufferVolumeDataUpdateBounds5D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
...
! test: check for pointer when there is a pointer
  ...
  CALL CheckForPointerToVolumeData(dataVolumeObject     = int8DataVolume5DObject, &
...
! cause an error:  try to destroy an object with an outstanding pointer
  ...
  CALL DestroyObject(object            = int8DataVolume5DObject,  &
                     nodeMPIDataObject = mpiData,                    &
                     enableChecking    = enableChecking,             &
                     errorInfoObject   = errorInfoObject)
...
! remove the pointer and try again
  ...
  CALL RemovePointerToVolumeData(object               = int8DataVolume5DObject, &
                                 pointerToVolumeData  = int8Ptr5D,              &
                                 nodeMPIDataObject    = mpiData,                   &
                                 enableChecking       = enableChecking,            &
                                 errorInfoObject      = errorInfoObject)
...
! cause an error:  try to destroy an object that's already been destroyed.
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '-------------------------------------------'
   WRITE(0,*) 'Testing int8DataVolume5DObject complete.'
   WRITE(0,*) '-------------------------------------------'
  END IF
