...
!--------------create a dataVolume5DClass object------------------------------
  ...
  CALL CreateObject(object                       = real4DataVolume5DObject,             &
                    bufferVolumeDataTotalBounds  = real4BufferVolumeDataTotalBounds5D,  &
                    bufferVolumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds5D, &
                    bufferVolumeData             = real4Data5D,                         &
...
! Test the data in the object
  ...
  CALL GetPointerToVolumeData(object                  = real4DataVolume5DObject,              &
                              pointerToVolumeData     = real4Ptr5D,                           &
                              pointerDataTotalBounds  = real4PointerVolumeDataTotalBounds5D,  &
                              pointerDataUpdateBounds = real4PointerVolumeDataUpdateBounds5D, &
                              nodeMPIDataObject       = mpiData,                                &
                              enableChecking          = enableChecking,                         &
                              errorInfoObject         = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   WRITE(0,*) '--------------------------------'
   WRITE(0,*) 'Checking data: real4Ptr5D bounds are: '
   WRITE(0,*) '  dimension 1: ',LBOUND(real4Ptr5D,1),UBOUND(real4Ptr5D,1)
   WRITE(0,*) '  dimension 2: ',LBOUND(real4Ptr5D,2),UBOUND(real4Ptr5D,2)
   WRITE(0,*) '  dimension 3: ',LBOUND(real4Ptr5D,3),UBOUND(real4Ptr5D,3)
   WRITE(0,*) '  dimension 4: ',LBOUND(real4Ptr5D,4),UBOUND(real4Ptr5D,4)
   WRITE(0,*) '  dimension 5: ',LBOUND(real4Ptr5D,5),UBOUND(real4Ptr5D,5)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4Ptr5D,        &
                      expectedDataValue = real4Data5D,       &
  ...
  IF (.NOT. correctValue) THEN
   ...
   charStringObject%charString = 'ERROR:  incorrect real4Data5D value'
   CALL AddErrorInformation(object          = errorInfoObject, &
                            errorInfoString = charStringObject)
  END IF
...
! get the volume data bounds and check
  ...
  CALL GetVolumeDataBounds(dataVolumeObject       = real4DataVolume5DObject,                 &
                           volumeDataTotalBounds  = real4BufferVolumeDataTotalBounds5DTest,  &
                           volumeDataUpdateBounds = real4BufferVolumeDataUpdateBounds5DTest, &
                           enableChecking         = enableChecking,                            &
                           errorInfoObject        = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataTotalBounds5DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataTotalBounds5DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real4BufferVolumeDataTotalBounds5DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',real4BufferVolumeDataTotalBounds5DTest(4,1:2)
   WRITE(0,*) '  dimension 5: ',real4BufferVolumeDataTotalBounds5DTest(5,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataTotalBounds5DTest, &
                      expectedDataValue = real4BufferVolumeDataTotalBounds5D,     &
                      correctValue      = correctValue,                             &
                      errorLocation     = errorLocation,                            &
                      errorInfoObject   = errorInfoObject)
  ...
  IF (masterNode .AND. verbose) THEN
   ...
   WRITE(0,*) '  dimension 1: ',real4BufferVolumeDataUpdateBounds5DTest(1,1:2)
   WRITE(0,*) '  dimension 2: ',real4BufferVolumeDataUpdateBounds5DTest(2,1:2)
   WRITE(0,*) '  dimension 3: ',real4BufferVolumeDataUpdateBounds5DTest(3,1:2)
   WRITE(0,*) '  dimension 4: ',real4BufferVolumeDataUpdateBounds5DTest(4,1:2)
   WRITE(0,*) '  dimension 5: ',real4BufferVolumeDataUpdateBounds5DTest(5,1:2)
   WRITE(0,*) '      No error is expected.'
  END IF
! check data

  CALL CheckDataValue(dataValue         = real4BufferVolumeDataUpdateBounds5DTest, &
                      expectedDataValue = real4BufferVolumeDataUpdateBounds5D,     &
                      correctValue      = correctValue,                              &
                      errorLocation     = errorLocation,                             &
                      errorInfoObject   = errorInfoObject)
