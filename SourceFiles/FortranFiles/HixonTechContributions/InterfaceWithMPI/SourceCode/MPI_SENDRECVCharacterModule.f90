...
MODULE MPI_SENDRECVCharacterCall
...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvCharacter

CONTAINS

SUBROUTINE MPISendRecvCharacter(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvCharacter

END MODULE MPI_SENDRECVCharacterCall

MODULE MPI_SENDRECVCharacterModule
  USE MPI_SENDRECVCharacterCall
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVCharacter
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVCharacter(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  CHARACTER, DIMENSION(:), INTENT(IN)  :: SENDBUF
  CHARACTER, DIMENSION(:), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvCharacter(SENDBUF   = SENDBUF,   &
                            SENDCOUNT = SENDCOUNT, &
                            SENDTYPE  = SENDTYPE,  &
                            DEST      = DEST,      &
                            SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVCharacter

END MODULE MPI_SENDRECVCharacterModule
