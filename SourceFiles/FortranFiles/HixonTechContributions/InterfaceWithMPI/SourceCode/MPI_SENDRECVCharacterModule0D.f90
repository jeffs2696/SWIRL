...
MODULE MPI_SENDRECVCharacterCall0D
...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvCharacter0D

CONTAINS

SUBROUTINE MPISendRecvCharacter0D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvCharacter0D

END MODULE MPI_SENDRECVCharacterCall0D

MODULE MPI_SENDRECVCharacterModule0D
  USE MPI_SENDRECVCharacterCall0D
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVCharacter0D
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVCharacter0D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  CHARACTER, INTENT(IN)  :: SENDBUF
  CHARACTER, INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvCharacter0D(SENDBUF   = SENDBUF,   &
                              SENDCOUNT = SENDCOUNT, &
                              SENDTYPE  = SENDTYPE,  &
                              DEST      = DEST,      &
                              SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVCharacter0D

END MODULE MPI_SENDRECVCharacterModule0D
