...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvInt4

CONTAINS

SUBROUTINE MPISendRecvInt4(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvInt4

END MODULE MPI_SENDRECVInt4Call

MODULE MPI_SENDRECVInt4Module
  USE MPI_SENDRECVInt4Call
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVInt4
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVInt4(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  INTEGER(KIND=int4Kind), DIMENSION(:), INTENT(IN)  :: SENDBUF
  INTEGER(KIND=int4Kind), DIMENSION(:), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvInt4(SENDBUF   = SENDBUF,   &
                          SENDCOUNT = SENDCOUNT, &
                          SENDTYPE  = SENDTYPE,  &
                          DEST      = DEST,      &
                          SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVInt4

END MODULE MPI_SENDRECVInt4Module
