...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvInt80D

CONTAINS

SUBROUTINE MPISendRecvInt80D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvInt80D

END MODULE MPI_SENDRECVInt8Call0D

MODULE MPI_SENDRECVInt8Module0D
  USE MPI_SENDRECVInt8Call0D
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVInt80D
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVInt80D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  INTEGER(KIND=int8Kind), INTENT(IN)  :: SENDBUF
  INTEGER(KIND=int8Kind), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvInt80D(SENDBUF   = SENDBUF,   &
                            SENDCOUNT = SENDCOUNT, &
                            SENDTYPE  = SENDTYPE,  &
                            DEST      = DEST,      &
                            SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVInt80D

END MODULE MPI_SENDRECVInt8Module0D
