...
MODULE MPI_SENDRECVLogicalCall
...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvLogical

CONTAINS

SUBROUTINE MPISendRecvLogical(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvLogical

END MODULE MPI_SENDRECVLogicalCall

MODULE MPI_SENDRECVLogicalModule
  USE MPI_SENDRECVLogicalCall
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVLogical
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVLogical(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  LOGICAL, DIMENSION(:), INTENT(IN)  :: SENDBUF
  LOGICAL, DIMENSION(:), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvLogical(SENDBUF   = SENDBUF,   &
                          SENDCOUNT = SENDCOUNT, &
                          SENDTYPE  = SENDTYPE,  &
                          DEST      = DEST,      &
                          SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVLogical

END MODULE MPI_SENDRECVLogicalModule
