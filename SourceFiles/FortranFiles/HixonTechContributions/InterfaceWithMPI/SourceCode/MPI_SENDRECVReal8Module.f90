...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvReal8

CONTAINS

SUBROUTINE MPISendRecvReal8(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvReal8

END MODULE MPI_SENDRECVReal8Call

MODULE MPI_SENDRECVReal8Module
  USE MPI_SENDRECVReal8Call
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVReal8
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVReal8(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  REAL(KIND=real8Kind), DIMENSION(:), INTENT(IN)  :: SENDBUF
  REAL(KIND=real8Kind), DIMENSION(:), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvReal8(SENDBUF   = SENDBUF,   &
                       SENDCOUNT = SENDCOUNT, &
                       SENDTYPE  = SENDTYPE,  &
                       DEST      = DEST,      &
                       SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVReal8

END MODULE MPI_SENDRECVReal8Module
