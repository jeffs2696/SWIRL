...
MODULE MPI_SENDRECVReal8Call0D
...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvReal80D

CONTAINS

SUBROUTINE MPISendRecvReal80D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvReal80D

END MODULE MPI_SENDRECVReal8Call0D

MODULE MPI_SENDRECVReal8Module0D
  USE MPI_SENDRECVReal8Call0D
...
INTERFACE HTMPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVReal80D
END INTERFACE HTMPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVReal80D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  REAL(KIND=real8Kind), INTENT(IN)  :: SENDBUF
  REAL(KIND=real8Kind), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvReal80D(SENDBUF   = SENDBUF,   &
                         SENDCOUNT = SENDCOUNT, &
                         SENDTYPE  = SENDTYPE,  &
                         DEST      = DEST,      &
                         SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVReal80D

END MODULE MPI_SENDRECVReal8Module0D
