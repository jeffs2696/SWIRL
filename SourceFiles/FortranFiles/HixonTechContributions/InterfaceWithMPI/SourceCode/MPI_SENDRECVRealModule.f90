...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvReal

CONTAINS

SUBROUTINE MPISendRecvReal(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvReal

END MODULE MPI_SENDRECVRealCall

MODULE MPI_SENDRECVRealModule
  USE MPI_SENDRECVRealCall
...
INTERFACE MPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVReal
END INTERFACE MPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVReal(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  REAL, DIMENSION(:), INTENT(IN)  :: SENDBUF
  REAL, DIMENSION(:), INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvReal(SENDBUF   = SENDBUF,   &
                       SENDCOUNT = SENDCOUNT, &
                       SENDTYPE  = SENDTYPE,  &
                       DEST      = DEST,      &
                       SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVReal

END MODULE MPI_SENDRECVRealModule
