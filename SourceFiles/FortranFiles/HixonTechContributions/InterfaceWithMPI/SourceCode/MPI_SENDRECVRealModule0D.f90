...
! this module actually makes the F77 MPI_SENDRECV call for character data
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: MPISendRecvReal0D

CONTAINS

SUBROUTINE MPISendRecvReal0D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
  ...
  RETURN
END SUBROUTINE MPISendRecvReal0D

END MODULE MPI_SENDRECVRealCall0D

MODULE MPI_SENDRECVRealModule0D
  USE MPI_SENDRECVRealCall0D
...
INTERFACE MPI_SENDRECV
  MODULE PROCEDURE MPI_SENDRECVReal0D
END INTERFACE MPI_SENDRECV

CONTAINS

SUBROUTINE MPI_SENDRECVReal0D(SENDBUF,SENDCOUNT,SENDTYPE,DEST,SENDTAG, &
           RECVBUF,RECVCOUNT,RECVTYPE,SOURCE,RECVTAG,COMM,STATUS,IERROR)
  REAL, INTENT(IN)  :: SENDBUF
  REAL, INTENT(OUT) :: RECVBUF
  INTEGER, INTENT(IN)  :: SENDCOUNT, RECVCOUNT
  ...
  CALL MPISendRecvReal0D(SENDBUF   = SENDBUF,   &
                         SENDCOUNT = SENDCOUNT, &
                         SENDTYPE  = SENDTYPE,  &
                         DEST      = DEST,      &
                         SENDTAG   = SENDTAG,   &
  ...
  RETURN
END SUBROUTINE MPI_SENDRECVReal0D

END MODULE MPI_SENDRECVRealModule0D
