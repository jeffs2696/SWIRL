...
! if dISweep(2) = +1, then the A+ matrix is added at j-1
   ...
   DO i=iMinUpdate(1),iMaxUpdate(1)
    DO n2=1,numberOfVariables
     DO n1=1,numberOfVariables
      abMatrixLast(n1,n2) = jFacA*aMatrixEta(n1,n2,i,jj,k) & ! +/-(1/2)*[A(j-/+1)]
                                 +bMatrixEta(n1,n2,i,jj,k)
     END DO
     abMatrixLast(n2,n2) = abMatrixLast(n2,n2) + 0.5_rDef*epsilonEta(i,jj,k) ! A+/- +/- (1/2)*epsilon(j-/+1)
!    abMatrixLast(n2,n2) = abMatrixLast(n2,n2) + jFacA*epsilonEta(i,jj,k) ! A+/- +/- (1/2)*epsilon(j-/+1)
    END DO

    dQLastEta(:,i) = MATMUL(abMatrixLast,deltaQ(:,i,jj,k))
