...
! do the xi sweep at j = jMinSweep
  ...
  include 'XiSweep4DInviscid2.f90'
...
! if dISweep(2) = +1, then the A+ matrix is added at j-1
   ...
   DO i=iMinUpdate(1),iMaxUpdate(1)
    DO n2=1,numberOfVariables
     DO n1=1,numberOfVariables
      aMatrixLast(n1,n2) = jFacA*aMatrixEta(n1,n2,i,jj,k,l) ! +/-(1/2)*[A(j-/+1)]
     END DO
     aMatrixLast(n2,n2) = aMatrixLast(n2,n2) + 0.5_rDef*epsilonEta(i,jj,k,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
!    aMatrixLast(n2,n2) = aMatrixLast(n2,n2) + jFacA*epsilonEta(i,jj,k,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
    END DO

    dQLastEta(:,i) = MATMUL(aMatrixLast,deltaQ(:,i,jj,k,l))
