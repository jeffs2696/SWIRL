   ...
   DO i=iMinUpdate(1),iMaxUpdate(1)
    DO n2=1,numberOfVariables
     ...
     abMatrixLast(n2,n2) = abMatrixLast(n2,n2)  &
                         + 0.5_rDef*epsilonEta(i,jj,k,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
!                        + jFacA*epsilonEta(i,jj,k,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
    END DO

    dQLastEta(:,i) = MATMUL(abMatrixLast,dQ(:,i,jj,k,l))
