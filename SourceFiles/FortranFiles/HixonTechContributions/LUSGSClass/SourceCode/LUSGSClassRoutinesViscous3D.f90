...
SUBROUTINE LUSGSViscous3DE3D(object,              & ! 
                             ...
                             bMatrixXi,           & ! (nV,nV,nI,nJ,nK)
                             iStartBMatrixEta,    & ! (nD+2)
                             bMatrixEta,          & ! (nV,nV,nI,nJ,nK)
                             iStartBMatrixZeta,   & ! (nD+2)
                             bMatrixZeta,         & ! (nV,nV,nI,nJ,nK)
                             iStartDeltaSigma,    & ! (nD)
                             deltaSigma,          & ! (nI,nJ,nK)
                             iStartDeltaQ,        & ! (nD+1)
                             deltaQ,              & ! (nV,nI,nJ,nK)
...
! define incoming data
  ...
  REAL(KIND=rDef), DIMENSION(iStartBMatrixXi(1):, &
                             ...
                             iStartBMatrixXi(3):, &
                             iStartBMatrixXi(4):, &
                             iStartBMatrixXi(5):), INTENT(IN) :: bMatrixXi
  ...
  REAL(KIND=rDef), DIMENSION(iStartBMatrixEta(1):, &
                             ...
                             iStartBMatrixEta(3):, &
                             iStartBMatrixEta(4):, &
                             iStartBMatrixEta(5):), INTENT(IN) :: bMatrixEta
  ...
  REAL(KIND=rDef), DIMENSION(iStartBMatrixZeta(1):, &
                             ...
                             iStartBMatrixZeta(3):, &
                             iStartBMatrixZeta(4):, &
                             iStartBMatrixZeta(5):), INTENT(IN) :: bMatrixZeta
...
! local variables
  ...
  REAL(KIND=rDef) :: iFacA,jFacA,kFacA

  CHARACTER(LEN=*), PARAMETER :: location = &
    'SUBROUTINE LUSGSViscous3DE3D'
...
! sweep 1
  ...
  include 'ZetaSweep3DViscous1.f90'

! sweep2
  
  include 'ZetaSweep3DViscous2.f90'
...
END SUBROUTINE LUSGSViscous3DE3D
