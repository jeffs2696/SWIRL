...
SUBROUTINE LUSGSViscous4DE4D(object,              & ! 
                             ...
                             bMatrixXi,           & ! (nV,nV,nI,nJ,nK,nL)
                             iStartBMatrixEta,    & ! (nD+2)
                             bMatrixEta,          & ! (nV,nV,nI,nJ,nK,nL)
                             iStartBMatrixZeta,   & ! (nD+2)
                             bMatrixZeta,         & ! (nV,nV,nI,nJ,nK,nL)
                             iStartBMatrixTau,    & ! (nD+2)
                             bMatrixTau,          & ! (nV,nV,nI,nJ,nK,nL)
                             iStartDeltaSigma,    & ! (nD)
...
! define incoming data
  ...
  REAL(KIND=rDef), DIMENSION(iStartBMatrixXi(1):, &
                             ...
                             iStartBMatrixXi(4):, &
                             iStartBMatrixXi(5):, &
                             iStartBMatrixXi(6):), INTENT(IN) :: bMatrixXi
  ...
  REAL(KIND=rDef), DIMENSION(iStartBMatrixEta(1):, &
                             ...
                             iStartBMatrixEta(4):, &
                             iStartBMatrixEta(5):, &
                             iStartBMatrixEta(6):), INTENT(IN) :: bMatrixEta

  INTEGER, DIMENSION(nD+2), INTENT(IN) :: iStartBMatrixZeta
  REAL(KIND=rDef), DIMENSION(iStartBMatrixZeta(1):, &
                             iStartBMatrixZeta(2):, &
                             iStartBMatrixZeta(3):, &
                             iStartBMatrixZeta(4):, &
                             iStartBMatrixZeta(5):, &
                             iStartBMatrixZeta(6):), INTENT(IN) :: bMatrixZeta

  INTEGER, DIMENSION(nD+2), INTENT(IN) :: iStartBMatrixTau
  REAL(KIND=rDef), DIMENSION(iStartBMatrixTau(1):, &
                             iStartBMatrixTau(2):, &
                             iStartBMatrixTau(3):, &
                             iStartBMatrixTau(4):, &
                             iStartBMatrixTau(5):, &
                             iStartBMatrixTau(6):), INTENT(IN) :: bMatrixTau
...
! local variables
  ...
  REAL(KIND=rDef) :: iFacA,jFacA,kFacA,lFacA

  CHARACTER(LEN=*), PARAMETER :: location = &
    'SUBROUTINE LUSGSViscous4DE4D'
...
! sweep 1
  ...
  include 'TauSweep4DViscous1.f90'

! sweep2
  
  include 'TauSweep4DViscous2.f90'
...
END SUBROUTINE LUSGSViscous4DE4D
