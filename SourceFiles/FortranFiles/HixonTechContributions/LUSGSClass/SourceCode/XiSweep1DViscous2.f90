  i  = iMinSweep(1,2)
  iFacA = 0.5_rDef*REAL(dISweep(2),rDef) ! +1/2 for + sweep, -1/2 for - sweep
...
! and do the rest of the xi sweep
  ...
  DO i = iMinSweep(1,2)+dISweep(2),iMaxSweep(1,2),dISweep(2)
   ii = i - dISweep(2) ! i+1 for - sweep, i-1 for + sweep

! if dISweep(1) = -1, then the A- matrix is subtracted at i+1 (+ eps)
! if dISweep(1) = +1, then the A+ matrix is added at i-1 ( + eps)
...
!                       + iFacA*epsilonXi(ii) ! A+/- +/- (1/2)*epsilon(i-/+1)
   ...
   dQLastXi(:) = MATMUL(abMatrixLast,deltaQ(:,ii))
...
!  solution in dQCurrent
   ...
   DO nV=1,numberOfVariables
    deltaQ(nV,i) = dQ(nV,i) + dQCurrent(nV)
   END DO
