...
! do the xi sweep at k = kMinSweep
  ...
  include 'EtaSweep3DViscous2.f90'
   ...
   DO j=iMinUpdate(2),iMaxUpdate(2)
    DO i=iMinUpdate(1),iMaxUpdate(1)
     ...
     DO n2=1,numberOfVariables
      DO n1=1,numberOfVariables
       abMatrixLast(n1,n2) = kFacA*aMatrixZeta(n1,n2,i,j,kk) & ! +/-(1/2)*[A(j-/+1)]
                                  +bMatrixZeta(n1,n2,i,j,kk) 
      END DO
      abMatrixLast(n2,n2) = abMatrixLast(n2,n2) + 0.5_rDef*epsilonZeta(i,j,kk) ! A+/- +/- (1/2)*epsilon(j-/+1)
!     abMatrixLast(n2,n2) = abMatrixLast(n2,n2) + kFacA*epsilonZeta(i,j,kk) ! A+/- +/- (1/2)*epsilon(j-/+1)
     END DO

     dQLastZeta(:,i,j) = MATMUL(abMatrixLast,deltaQ(:,i,j,kk))
