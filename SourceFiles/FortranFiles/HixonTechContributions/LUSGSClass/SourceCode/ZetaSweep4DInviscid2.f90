...
! do the xi sweep at k = kMinSweep
  ...
  include 'EtaSweep4DInviscid2.f90'
   ...
   DO j=iMinUpdate(2),iMaxUpdate(2)
    DO i=iMinUpdate(1),iMaxUpdate(1)
     ...
     DO n2=1,numberOfVariables
      DO n1=1,numberOfVariables
       aMatrixLast(n1,n2) = kFacA*aMatrixZeta(n1,n2,i,j,kk,l) ! +/-(1/2)*[A(j-/+1)]
      END DO
      aMatrixLast(n2,n2) = aMatrixLast(n2,n2) + 0.5_rDef*epsilonZeta(i,j,kk,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
!     aMatrixLast(n2,n2) = aMatrixLast(n2,n2) + kFacA*epsilonZeta(i,j,kk,l) ! A+/- +/- (1/2)*epsilon(j-/+1)
     END DO

     dQLastZeta(:,i,j) = MATMUL(aMatrixLast,deltaQ(:,i,j,kk,l))
