#!/usr/bin/env python3
# - [ ] add number of grid points to plot
# - [ ] plot each dissipation on its own
# - [ ] export plots
# - [ ] obtain results for second order 
import sys
sys.path.append("..")
...
for i_gp,j_gp in enumerate(grid_point_array):
    ...
    x_data_list = []
    for i in range(0,5): 
        filename = f'SWIRL_Dissipation_nD_{i+1}_{j_gp}_fd2.dat'.format()
        analytic_filename = f'../analytical_mode_data_{j_gp}.dat'.format()
        numeric_filename =f'../numerical_mode_data_{j_gp}.dat'.format() 
        # print(filename)
        analytic_data = \
                pd.read_csv(analytic_filename, delim_whitespace = True, header= None) 

        numeric_data = \
                pd.read_csv(numeric_filename, delim_whitespace = True, header= None) 
        ...
        y_data_list.append(abs(analytic_data[1]-numeric_data[1]))

        y2_data_list.append(abs(analytic_data[1] -(data['dissipation']+data['no_dissipation'])))
        ...
        grid = x_data_list
        # error = y_data_list
        error = abs(analytic_data[1]-numeric_data[1])
    ...
    # if plot_mode_data_flag: 
        ...
        # plt.plot(
        #         x_data_list[i],
        #         abs(numeric_data[1]-analytic_data[1]))
        ...
        # plt.plot(
        #         x_data_list[i],
        #         # abs(analytic_data[1] - (data['dissipation'] + data['no_dissipation'])),
        #         ((y2_data_list[i])),
        #         label = 'dissipated')
        # plt.show()
    # sys.exit()
    ...
    L2_list.append(np.sqrt(sum(1/len(error)*error**2)))
