...
def main(): 
    ...
    if debug:
        logger.info(f'Sorting SWIRL data for each grid...')
    ...
    m_dict_list.append(m_dict_second_order)
    m_dict_list.append(m_dict_fourth_order)

    if debug:
        logger.info(f'Finished importing axial wavenumbers...')
        logger.info(f'Importing the corresponding pressure modes...')

    for i_dict,j_dict in enumerate(m_dict_list):
        for i_gp,j_gp in enumerate(grid_point_array): 
            radial_mode_filenames = []
            m_dict = m_dict_list[i_dict][i_gp]
            ...
            for i in range(len(radial_mode_filenames)):
            ...
            m_dict['radial_mode_data'] = radial_mode_dataframe_dictionary


            # sys.exit()

            if debug:
                logger.info(f'Finished importing pressure modes...')
                logger.info(f'Computing the corresponding analytic solution...')
            ...
            # if debug:
                # logger.info('Normalizing radial mode shapes...')
...
# the one uptop
            ...
            if plot_figures_flag:
                ...
                # - [ ] add legend
                # - [ ] specify the finite diff method UseD in plot by Changing m_diCt ddefn
                fig,ax = plt.subplots(
    ...
    # for i_gp,j_gp in enumerate(grid_point_array): 
        radial_mode_filenames = []
        for i in range(len(m_dict_fourth_order[i_gp]['radial_mode_index'])):
           # this concept is flawed .... it expects a certain array of grid points
           #
           # m_dict = m_dict_fourth_order[i_gp]
           if i_gp == 0:
               if m_dict_fourth_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append(
                           'egv_np_00{n}radialmode_00'.format(
                               n = j_gp) +
                           str(m_dict_fourth_order[i_gp]['radial_mode_index'][i])) 
               else:
                   radial_mode_filenames.append(
                           'egv_np_00{n}radialmode_0'.format(
                               n = j_gp ) +
           ...
           if i_gp == 1:
               if m_dict_fourth_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append('egv_np_00{n}radialmode_00'.format(
                       n = j_gp)+ str(m_dict_fourth_order[i_gp]['radial_mode_index'][i])) 
               else:
                   radial_mode_filenames.append('egv_np_00{n}radialmode_0'.format(
                       n = j_gp) + str(m_dict_fourth_order[i_gp]['radial_mode_index'][i]))
           if i_gp == 2:
           ...
           if i_gp == 4:
               if m_dict_fourth_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_00'.format(
                       n = j_gp) + str(m_dict_fourth_order[i_gp]['radial_mode_index'][i]))
               elif m_dict_fourth_order[i_gp]['radial_mode_index'][i] < 1000:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_0'.format(
                       n = j_gp) + str(m_dict_fourth_order[i_gp]['radial_mode_index'][i]))
               else:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_'.format(
        ...
        m_dict_fourth_order[i_gp]['radial_mode_data'] = radial_mode_dataframe_dictionary

        radial_mode_filenames = []
        for i in range(len(m_dict_second_order[i_gp]['radial_mode_index'])):
           # this concept is flawed .... it expects a certain array of grid points
           #
           # m_dict = m_dict_fourth_order[i_gp]
           if i_gp == 0:
               if m_dict_second_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append(
                           'egv_np_00{n}radialmode_00'.format(
                               n = j_gp) +
                           str(m_dict_second_order[i_gp]['radial_mode_index'][i])) 
               else:
                   radial_mode_filenames.append(
                           'egv_np_00{n}radialmode_0'.format(
                               n = j_gp ) +
           ...
           if i_gp == 1:
               if m_dict_second_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append('egv_np_00{n}radialmode_00'.format(
                       n = j_gp)+ str(m_dict_second_order[i_gp]['radial_mode_index'][i])) 
               else:
                   radial_mode_filenames.append('egv_np_00{n}radialmode_0'.format(
                       n = j_gp) + str(m_dict_second_order[i_gp]['radial_mode_index'][i]))
           if i_gp == 2:
           ...
           if i_gp == 4:
               if m_dict_second_order[i_gp]['radial_mode_index'][i] < 100:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_00'.format(
                       n = j_gp) + str(m_dict_second_order[i_gp]['radial_mode_index'][i]))
               elif m_dict_second_order[i_gp]['radial_mode_index'][i] < 1000:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_0'.format(
                       n = j_gp) + str(m_dict_second_order[i_gp]['radial_mode_index'][i]))
               else:
                   radial_mode_filenames.append('egv_np_0{n}radialmode_'.format(
        ...
        for i in range(len(radial_mode_filenames)):
            ...
            radial_mode_data = \
                    pandas.read_csv(  second_order_directories[i_gp] +
                            radial_mode_filenames[i], delim_whitespace = True )
            radial_mode_dataframe_dictionary[i] = radial_mode_data

        m_dict_second_order[i_gp]['radial_mode_data'] = radial_mode_dataframe_dictionary
